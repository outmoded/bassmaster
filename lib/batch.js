    ...
    handler: async function(request, h) {
      ...
      request.payload.requests.every((req, idx) => {
        ...
        if (result === "") {
          ...
          errorMessage =
            errorMessage || `Invalid request format in item: ${idx}`;
          return false;
        }
...
internals.evalResults = function(results, index, path) {
  ...
  let evaluatedResult = Hoek.reach(result, path);
...
internals.buildPayload = function(payload, resultsData, parts) {
  ...
  for (let i = 0; i < partsLength; ++i) {
    part = parts[i];
    result = internals.evalResults(
      resultsData.resultsMap,
      part.resultIndex,
      part.resultPath
    );
...
internals.batch = async function(
  batchRequest,
  resultsData,
  pos,
  requestParts,
