...
internals.batch = function (request, resultsData, pos, parts, callback) {
    ...
    if (error === null) {
        ...
        // Make request
        request.payload.requests[pos].path = path;
        internals.dispatch(request, request.payload.requests[pos], function (data) {
...
internals.dispatch = function (batchRequest, request, callback) {
    ...
    var body = (request.payload !== null ? JSON.stringify(request.payload) : null);     // content can be '' or 0
    var injectOptions = {
        url: request.path,
