...
internals.batch = function (request, resultsData, pos, parts, callback) {
    ...
    if (error === null) {
        ...
        internals.dispatch(request, request.payload.requests[pos].method, path, request.payload.requests[pos].payload, function (data) {
...
internals.dispatch = function (request, method, path, payload, callback) {

    var body = (payload !== null ? JSON.stringify(payload) : null);     // content can be '' or 0
    var injectOptions = {
        url: path,
        method: method,
        payload: body,
