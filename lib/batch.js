    ...
    return {
        handler: function (request, reply) {
            ...
            for (var i = 0, il = request.payload.requests.length; i < il; ++i) {
                ...
                var payloadParts = internals.parsePayload(payload);
...
internals.evalResults = function (resultsMap, resultsPath, resultsPathRe) {

    var match = resultsPath.match(resultsPathRe);
    var index = match[1];
    var path = resultsPath.substr(match[0].length);
    var value = resultsMap[index];

    if (path.charAt(0) === '.') {
        value = Hoek.reach(value, path.substr(1));
    }
...
internals.buildPayload = function (payload, resultsData, parts) {
    ...
    for (var i = 0, il = parts.length; i < il; ++i) {
        ...
        var data = internals.evalResults(resultsData.resultsMap, parts[i].value, internals.payloadRegex);

        if (parts[i].path.length) {
            Traverse(payload).set(parts[i].path, data);
        } else {
            payload = data;
        }
    }
